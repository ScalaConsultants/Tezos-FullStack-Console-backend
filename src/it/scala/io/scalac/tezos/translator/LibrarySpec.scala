...
import io.scalac.tezos.translator.routes.dto.LibraryEntryRoutesDto.libraryEntryRoutesDtoDecoder
import io.circe._
...
class LibrarySpec extends WordSpec with Matchers with ScalatestRouteTest with ScalaFutures with BeforeAndAfterEach {
  ...
  val libraryEntryId: LibraryEntryId    = LibraryEntryId(refineMV("c77af309-a37e-4e41-895b-468c818c6082"))

  val title     = "AAA"
  val author    = "BBB"
  val email     = "test@email.com"
  val desc      = "test-desc"
  val micheline = "test-micheline"
  val michelson = "test-michelson"

  val jsonProperPayload =
    s"""{"title": "AAA", "author": "BBB", "email": "test@email.com", "description": "test-desc", "micheline": "test-micheline", "michelson": "test-michelson"}""".stripMargin

  val libraryRoute: Route =
    new LibraryRoutes(libraryService, userService, email2SendService, system.log, reCaptchaConfig, adminEmail).routes

  def insertDummiesToDb(size: Int, status: Option[Status] = Some(Accepted)): Future[immutable.IndexedSeq[Int]] = {
    val inserts = for {
      i <- 1 to size
      dummyData = LibraryEntry(generateLibraryEntryId,
                               properTitle,
                               properAuthor,
                               properEmail,
                               properDescription,
  ...
  "Library routes" should {
    ...
    "store proper payload" should {
      ...
      "parser test  " in {
        ...
        val jsonAsLibraryEntryRouteDto: LibraryEntryRoutesDto = parse(jsonProperPayload).flatMap(_.as[LibraryEntryRoutesDto]).right.get
        jsonAsLibraryEntryRouteDto.title.toString shouldBe title
        jsonAsLibraryEntryRouteDto.author.get.toString shouldBe author
        jsonAsLibraryEntryRouteDto.email.get.toString shouldBe email
        jsonAsLibraryEntryRouteDto.description.get.toString shouldBe desc
        jsonAsLibraryEntryRouteDto.micheline.toString shouldBe micheline
        jsonAsLibraryEntryRouteDto.michelson.toString shouldBe michelson
      }

      "full filed payload " in {
        val titleForFilter = Title(refineMV[NotEmptyAndNotLong]("AAA"))

        Post(libraryEndpoint, parse(jsonProperPayload)) ~> Route.seal(libraryRoute) ~> check {
        ...
        addedRecord.title.toString shouldBe title
        addedRecord.author.get.toString shouldBe author
        addedRecord.email.get.toString shouldBe email
        addedRecord.description.get.toString shouldBe desc
      ...
      "payload with UpperCased Email make lower " in {
        ...
        val jsonUpperCaseEmailPayload: String =
          s"""{"title": "AAA", "author": "BBB", "email": "Test@email.com", "description": "test-desc", "micheline": "test-micheline", "michelson": "test-michelson"}""".stripMargin

        Post(libraryEndpoint, parse(jsonUpperCaseEmailPayload)) ~> Route.seal(libraryRoute) ~> check {
          status shouldBe StatusCodes.OK
        }
        val dbRequest: Future[Seq[LibraryEntryDbDto]] = testDb.run(LibraryTable.library.filter(_.title === titleForFilter).result)
        val currentLibrary: Seq[LibraryEntryDbDto]    = Await.result(dbRequest, 1 second)
    ...
    "add emails to queue when new library entry added and status changed" in {
      ...
      val record = LibraryEntryRoutesDto(libraryEntryId,
  ...
  "correctly filter records" in new SampleEntries {
    ...
    val expectedRecord2 = LibraryEntryRoutesDto(LibraryEntryId(refineMV("fd5c504b-60eb-464f-ba64-14988f1fccd0")),
    ...
    Get(libraryEndpoint) ~> libraryRoute ~> check {
      ...
      actualRecords.head.email shouldBe expectedRecord2.email // uid is random generated
      actualRecords.head.micheline shouldBe expectedRecord2.micheline
      actualRecords.head.michelson shouldBe expectedRecord2.michelson
      actualRecords.head.description shouldBe expectedRecord2.description
      actualRecords.head.author shouldBe expectedRecord2.author
      actualRecords.head.title shouldBe expectedRecord2.title
    }
  }
  ...
  "update library entry status" in new SampleEntries {
    ...
    val expectedRecord2 = LibraryEntryRoutesDto(libraryEntryId,
                                                Title(refineMV[NotEmptyAndNotLong]("nameE2")),
                                                Author(refineMV[NotEmptyAndNotLong]("authorE2")).some,
                                                Some(EmailS(refineMV[EmailReq]("name@service.com"))),
                                                Description(refineMV[NotEmptyAndNotLong]("descriptionE2")).some,
    ...
    Get(libraryEndpoint) ~> libraryRoute ~> check {
      status shouldBe StatusCodes.OK
      println(response) // check why here is empty repsonse and in delete is not
      val actualRecords = responseAs[List[LibraryEntryRoutesDto]]
      actualRecords.head.email shouldBe expectedRecord2.email // uid is  skipped because is random generated, i could use copy but in this case it's only one element
      actualRecords.head.micheline shouldBe expectedRecord2.micheline
      actualRecords.head.michelson shouldBe expectedRecord2.michelson
      actualRecords.head.description shouldBe expectedRecord2.description
      actualRecords.head.author shouldBe expectedRecord2.author
    ...
    val expectedRecord1 = LibraryEntryRoutesDto(libraryEntryId,
    ...
    Get(libraryEndpoint) ~> libraryRoute ~> check {
      status shouldBe StatusCodes.OK
      val actualRecords = responseAs[List[LibraryEntryRoutesDto]]
      actualRecords.head.copy(uid = libraryEntryId) shouldBe expectedRecord1.copy(uid = libraryEntryId)
    }
  }
...
//test if the same data gated from database is the same after two requests
  ...
  private trait SampleEntries {
    ...
    val record1 = LibraryEntry(LibraryEntryId(refineMV[Uuid]("d7327913-4957-4417-96d2-e5c1d4311f80")),
                               Title(refineMV[NotEmptyAndNotLong]("nameE1")),
                               None,
                               None,
                               None,
    ...
    val record2 = LibraryEntry(LibraryEntryId(refineMV[Uuid]("17976f3a-505b-4d66-854a-243a70bb94c0")),
                               Title(refineMV[NotEmptyAndNotLong]("nameE2")),
                               Author(refineMV[NotEmptyAndNotLong]("authorE2")).some,
    ...
    val record3 = LibraryEntry(LibraryEntryId(refineMV[Uuid]("5d8face2-ab24-49e0-b792-a0b99a031645")),
