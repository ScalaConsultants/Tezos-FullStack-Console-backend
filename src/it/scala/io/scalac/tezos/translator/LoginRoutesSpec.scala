...
class LoginRoutesSpec
  ...
  with BeforeAndAfterEach {
    ...
    "LoginRoute" should "reject wrong credentials" in {
      Post(loginEndpoint, UserCredentials("admin", "asdf")) ~> loginRoute ~> check {
        status shouldBe StatusCodes.Forbidden
      }
    }

    it should "accept correct credentials and return token" in {
      Post(loginEndpoint, UserCredentials("admin", "zxcv")) ~> loginRoute ~> check {
    ...
    it should "handle non-existing username gracefully" in {
      Post(loginEndpoint, UserCredentials("zxcv", "zxcv")) ~> loginRoute ~> check {
        status shouldBe StatusCodes.Forbidden
      }
    }
