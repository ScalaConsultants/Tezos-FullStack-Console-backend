...
class LoginRoutesSpec extends FlatSpec with Matchers with ScalatestRouteTest with ScalaFutures with BeforeAndAfterEach {
  ...
  val loginRoute: Route                               = new LoginRoutes(new UserService(new UserRepository, tokenToUser, testDb)).routes
  ...
  it should "only allowed logout for users who are logged in" in {
    Post(logoutEndpoint).withHeaders(Authorization(OAuth2BearerToken("someRandomTokenWithLength30Sym"))) ~> Route
      .seal(loginRoute) ~> check {
