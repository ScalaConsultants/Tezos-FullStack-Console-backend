...
class LoginRouteSpec extends FlatSpec with Matchers with ScalatestRouteTest with JsonHelper with ForEachTestContainer {
  ...
  private trait DatabaseFixture extends DbTestBase {
    ...
    val loginRoute = new LoginRoute(new UserService(new UserRepository, testDb)).routes
