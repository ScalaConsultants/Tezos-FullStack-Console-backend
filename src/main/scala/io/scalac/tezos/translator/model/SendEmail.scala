...
object SendEmail {
  ...
  def approvalRequest(
    libraryDto: LibraryEntryRoutesDto,
  ...
  def fromSendEmailRoutesDto(
    dto: SendEmailRoutesDto,
    adminEmail: EmailAddress
  ): Try[SendEmail] =
    for {
  ...
  def fromSendEmailDbDto(dto: SendEmailDbDto): Try[SendEmail] =
    ...
    } yield new SendEmail(uid = uid, to = to, subject = dto.subject, content = c) {}
