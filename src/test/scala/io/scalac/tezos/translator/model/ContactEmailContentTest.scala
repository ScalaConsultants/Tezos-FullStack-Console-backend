...
class ContactEmailContentTest extends WordSpec with Matchers {
  "EmailContentTest" should {
    "to Json and from " in {
      val dummyContactFormContent: EmailContent = ContactFormContent("Test", FullContact(Phone(refineMV[PhoneReq]("+11223344")), EmailAddress.fromString("test@service.com").get), "TestContent")
