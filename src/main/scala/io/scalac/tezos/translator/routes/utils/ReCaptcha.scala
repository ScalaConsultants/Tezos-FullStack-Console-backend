...
object ReCaptcha {
  ...
  protected def captchaHeaderIsMissingAsFuture(captchaHeaderName: String): Future[Either[ErrorResponse, Unit]] =
    Future
      .successful((Error(s"Request is missing required HTTP header '$captchaHeaderName'"), StatusCode.BadRequest).asLeft)

  protected def checkCaptcha(
     userCaptcha: Captcha,
     log: LoggingAdapter,
     reCaptchaConfig: CaptchaConfig
   )(implicit actorSystem: ActorSystem,
