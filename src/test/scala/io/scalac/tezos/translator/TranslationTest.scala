...
class TranslationTest extends WordSpec with MustMatchers with ScalaFutures with ScalatestRouteTest {
  ...
  val routes: Route = new TranslatorRoutes(Fakes.MMTranslatorFake, log, config.reCaptcha).routes

  "A Routes" can {
    "translate michelson to micheline" when {
      "michelson is correct" in {
        Post("/translate/from/michelson/to/micheline", Samples.michelson) ~> routes ~> check {
      ...
      "michelson in incorrect 1" in {
        Post("/translate/from/michelson/to/micheline", Samples.incorrectMichelson1) ~> routes ~> check {
    ...
    "translate micheline to michelson" when {
      ...
      "micheline is correct" in {
        Post("/translate/from/micheline/to/michelson", Samples.micheline) ~> routes ~> check {
          status must equal(StatusCodes.OK)
      ...
      "micheline is incorrect 1" in {
        Post("/translate/from/micheline/to/michelson", Samples.incorrectMicheline1) ~> routes ~> check {
          status must equal(StatusCodes.BadRequest)
        }
      }

      "micheline is incorrect 2" in {
        Post("/translate/from/micheline/to/michelson", Samples.incorrectMicheline2) ~> routes ~> check {
          status must equal(StatusCodes.BadRequest)
        }
      }
