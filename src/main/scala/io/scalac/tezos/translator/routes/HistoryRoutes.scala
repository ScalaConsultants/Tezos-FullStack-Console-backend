...
class HistoryRoutes(translationsService: TranslationsService,
                    dbUtilityConfig: DBUtilityConfiguration) extends HttpRoutes with JsonSupport {

  val defaultResultLimit: Int = 10

  override def routes: Route =
    (pathPrefix("translations") & pathEndOrSingleSlash & get &
      parameters('source.as[Translation.From].?, 'limit.as[Int].?)){ (sourceOpt, limitOpt) =>
      val result = for {
        translations <- translationsService.list(sourceOpt, limitOpt.getOrElse(dbUtilityConfig.defaultLimit))
